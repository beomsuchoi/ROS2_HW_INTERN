...
void TurtlesimCliNode::set_background_color() {
    ...
    if (parameter_client->wait_for_service(std::chrono::seconds(10))) {  // 대기 시간 10초로 변경
...
void TurtlesimCliNode::set_turtle_shape() {
    ...
    if (kill_client_->wait_for_service(std::chrono::seconds(10))) {  // 대기 시간 10초로 변경
    ...
    if (spawn_client_->wait_for_service(std::chrono::seconds(10))) {  // 대기 시간 10초로 변경
...
void TurtlesimCliNode::set_pen() {
    ...
    std::cout << "0~10 넓이 값을 입력하세요: ";
    std::cin >> width;

    request->width = static_cast<uint8_t>(width);
    request->off = 0;

    if (set_pen_client_->wait_for_service(std::chrono::seconds(10))) {  // 대기 시간 10초로 변경
